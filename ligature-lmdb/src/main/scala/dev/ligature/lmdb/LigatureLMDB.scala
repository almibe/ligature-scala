...
class LigatureLMDB {
  ...
  def addEntries(
      name: String,
      entries: fs2.Stream[cats.effect.IO, dev.ligature.wander.Triple]
  ): fs2.Stream[cats.effect.IO, Unit] = ???

  def query(
      name: String,
      query: dev.ligature.wander.LigatureValue.Pattern,
      template: dev.ligature.wander.LigatureValue.Pattern | dev.ligature.wander.LigatureValue.Quote
