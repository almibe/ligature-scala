...
class ParserSuite extends FunSuite {
  ...
  def check(script: String): Either[WanderError, Seq[Seq[Term]]] =
  ...
  test("parse nothing keyword") {
    ...
    val expected = Right(Seq(Seq(Term.NothingLiteral)))
    assertEquals(result, expected)
  }
  test("parse Identifier") {
    val result = check("<test2>")
    val expected = Right(Seq(Seq(ident("test2"))))
    assertEquals(result, expected)
  }
  test("parse Integer") {
    val result = check("-321")
    val expected = Right(Seq(Seq(Term.IntegerLiteral(-321))))
    assertEquals(result, expected)
  }
  test("parse String") {
    val result = check("\"hello\"")
    val expected = Right(Seq(Seq(Term.StringLiteral("hello"))))
    assertEquals(result, expected)
  }
  test("parse Boolean") {
    val result = check("false")
    val expected = Right(Seq(Seq(Term.BooleanLiteral(false))))
    assertEquals(result, expected)
  }
  test("parse Function Calls") {
    val input = check("not true")
    val expected =
      Right(Seq(Seq(Term.NameTerm(Name("not")), Term.BooleanLiteral(true))))
    assertEquals(input, expected)
  }
  test("parse Function Call with question mark argument") {
    val result = check("query ?")
    val expected = Right(Seq(Seq(      
        Term.NameTerm(Name("query")), Term.QuestionMark
    )))
    assertEquals(result, expected)
  }
  test("parse empty List") {
    val result = check("[]")
    val expected = Right(Seq(Seq(Term.Array(Seq()))))
    assertEquals(result, expected)
  }
  test("parse List") {
  ...
  test("parse let expression") {
    ...
    val expected = Right(Seq(Seq(Term.LetExpression(Name("x"), Term.IntegerLiteral(5)))))
    assertEquals(result, expected)
  }
  test("parse Lambda") {
  ...
  test("parse let expression with lambda") {
    ...
    val expected =
      Right(Seq(Seq(Term.LetExpression(Name("id"), Term.Lambda(Seq(Name("x")), Term.NameTerm(Name("x")))))))
    assertEquals(result, expected)
  }
