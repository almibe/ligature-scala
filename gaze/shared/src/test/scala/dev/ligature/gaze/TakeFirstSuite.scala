...
class TakeFirstSuite extends FunSuite {
  ...
  val takeFirstEmpty: Nibbler[Char, String] = takeFirst()
  ...
  test("first match take first") {
    ...
    assertEquals(gaze.attempt(takeFirstEmpty), None)
    assertEquals(gaze2.attempt(takeFirstSingle), Some("hello"))
    assertEquals(gaze3.attempt(takeFirst3), Some("hello"))
  }
  ...
  test("middle match take first") {
    ...
    assertEquals(gaze.attempt(takeFirstEmpty), None)
    assertEquals(gaze2.attempt(takeFirstSingle), None)
    assertEquals(gaze3.attempt(takeFirst3), Some(" "))
  }
  ...
  test("last match take first") {
    ...
    assertEquals(gaze.attempt(takeFirstEmpty), None)
    assertEquals(gaze2.attempt(takeFirstSingle), None)
    assertEquals(gaze3.attempt(takeFirst3), Some("world"))
  }

  test("take first with repeats") {
    val gaze = Gaze.from("hellohellohello")
    assertEquals(gaze.attempt(takeFirstEmpty), None)
    assertEquals(gaze.attempt(takeFirstSingle), Some("hello"))
    assertEquals(gaze.attempt(takeFirst3), Some("hello"))
    assertEquals(gaze.attempt(takeFirst3), Some("hello"))
