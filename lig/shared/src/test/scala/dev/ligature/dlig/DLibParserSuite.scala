...
class DLigParserSuite extends FunSuite {
  ...
  test("id gen with prefix") {
    val input =
      parseIdentifier(Gaze.from("<this:is:a/prefix{}>"), Map()).getOrElse(???)
  ...
  test("id gen in infix") {
    val input =
      parseIdentifier(Gaze.from("<this{}is:a/infix>"), Map()).getOrElse(???)
    val resultRegEx = "this[0-9_\\-a-fA-F]{12}is:a/infix".r
    assert(resultRegEx.matches(input.name))
  }

  test("id gen in postfix") {
    val input = parseIdentifier(Gaze.from("<this::is:a/postfix/{}>"), Map())
      .getOrElse(???)
  ...
  test("prefixed id") {
    val input =
      parseIdentifier(Gaze.from("prefix:world"), Map("prefix" -> "hello:"))
        .getOrElse(???)
