...
class LigSuite : FunSpec() {
    init {
        ...
        test("basic Statement with all Entities") {
            ...
            val lines = lig.write(listOf(statement).iterator())
            val resStatements = lig.parse(lines)
            listOf(statement) shouldBe listOf(resStatements)
        }
        ...
        test("list of Statements with Literal Values") {
            ...
            val lines = lig.write(statements.iterator())
            val resStatements = lig.parse(lines)
            listOf(statements) shouldBe listOf(resStatements)
        }
        ...
        test("parsing with wildcards") {
            ...
            val resStatements = lig.parse(textInput)
            resStatements shouldBe expectedStatements
        }
    }
}
