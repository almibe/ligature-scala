...
// TODO: this function never returns an Error so if there is a malformed prefix it won't be caught
// until we try to read a Statement
fun parsePrefix(gaze: Gaze<Char>): Either<LigError, Pair<String, String>?> {
  val parseResult =
      gaze.attempt(
          takeAllGrouped(
              takeString("prefix"),
...
fun parseStatements(
  ...
  while (!gaze.isComplete) when (val res = parseStatement(gaze, prefixes, lastStatement)) {
...
fun genIdId(input: String): String {
  ...
  while (itr.hasNext()) when (val c = itr.next()) {
    '{' -> {
