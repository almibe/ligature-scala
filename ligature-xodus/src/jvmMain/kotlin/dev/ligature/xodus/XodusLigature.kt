...
enum class LigatureValueType(val id: Byte) {
  ...
  companion object {
    fun getValueType(id: Byte): LigatureValueType =
        when (id) {
...
class XodusLigature(private val dbDirectory: File) : Ligature, XodusOperations {
  ...
  /**
   * This method is ran once at the start to make sure all Stores exist. This is done so that Stores
   * can be opened in readonly mode. It also checks and initializes Counters TODO: This method could
   * probably check if the Environment exists and check the status of the Environment first.
   */
  private fun setupStores(): Unit {
    // create all Stores
    val createStoresTC: TransactionalComputable<Unit> =
        TransactionalComputable<Unit> { tx -> LigatureStore.values().forEach { openStore(tx, it) } }
    environment.computeInTransaction(createStoresTC)
    // set up counters -- for now it's just a single counter
    val checkCountersTC: TransactionalComputable<Unit> =
        TransactionalComputable<Unit> { tx ->
  ...
  /**
   * Used to uniformly open Stores. This is mainly so that StoreConfigs can be consistent. Right now
   * all stores use the same config but that is likely to change.
   */
  ...
  /**
   * Computes the next ID. Right now IDs are shared between all Stores but that might change. Also,
   * right now IDs are positive Long values and that might also change.
   */
  ...
  private fun fetchDatasetID(dataset: Dataset): ByteIterable? {
    val tc =
        TransactionalComputable<ByteIterable?> { tx ->
          val datasetToIdStore = openStore(tx, LigatureStore.DatasetToIdStore)
  ...
  /**
   * This method iterates through all the index stores and removes all Statements in a given
   * Dataset. TODO this method also needs to clean up all Identifiers, Strings, and Bytes
   */
