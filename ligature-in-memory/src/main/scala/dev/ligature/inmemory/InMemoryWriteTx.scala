...
class InMemoryWriteTx(private val store: DatasetStore) extends WriteTx {
    ...
    override def newAnonymousEntity(prefix: String): Uni[Either[LigatureError, Entity]] = ???//Uni {
...
//    }
    ...
    override def addStatement(statement: Statement, prefix: String): Uni[Either[LigatureError, PersistedStatement]] = ???//Uni {
...
//    }
     ...
     *  Note: Potentally could trigger a ValidationError. */
    def removeStatement(persistedStatement: PersistedStatement): Uni[Either[LigatureError, Boolean]] = ???//Uni {
...
//        Uni(Right(()))
     ...
     *  Used in the release method of the Resource[Uni, WriteTx]. */
