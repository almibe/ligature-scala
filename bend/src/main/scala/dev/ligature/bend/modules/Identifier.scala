  ...
  Map(
    Field("toString") -> BendValue.Function(
      HostFunction(
        ...
        (arguments, environment) => 
            arguments match
                case Seq(BendValue.Identifier(value)) => Right((BendValue.String(s"`${value.value}`"), environment))
                case _ => ???
      )
    ),
    Field("value") -> BendValue.Function(
      HostFunction(
        ...
        (arguments, environment) => 
            arguments match
                case Seq(BendValue.Identifier(value)) => Right((BendValue.String(value.value), environment))
                case _ => ???
      )
    ),
  )
