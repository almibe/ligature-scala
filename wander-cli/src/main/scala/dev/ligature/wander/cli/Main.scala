  ...
  run(script, environment) match
    ...
    case Right((value, _)) =>
      if isTest then
        ...
        println(printWanderValue(value))
...
def makeStyler(colors: List[String]): Styler = {
  new Styler {
    override def styleCell(column: com.github.freva.asciitable.Column, row: Int, col: Int, data: java.util.List[String]) : java.util.List[String] = {
